...
app.post("/checkout", connectDb, async (req, res, next) => {
  console.log("request body값 : "+JSON.stringify(req.body))
  const usestock = JSON.stringify(req.body.stock);
  const [ result ] = await req.conn.query(
    getProduct('CP-502101')
  )
  if (result.length > 0) {
    ...
    if (product.stock > 0) {
      ...
      return res.status(200).json({ message: `구매 완료! 남은 재고: ${product.stock - usestock}`});
    }
    else {
      ...
      const params = {
        ...
        MessageAttributes: {
          ...
          MessageAttributeFactoryId: {
            StringValue: `${req.body.factory_id}`,
            DataType: "String",
          },
